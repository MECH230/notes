...
\end{align}
These equations give ODE's for $x(t)$ and $y(t)$. Given IC's $x_0, \dot{x}_0, y_0, \dot{y}_0$, we have an initial value problem (IVP).

\item Equation (\ref{x_proj2}) is a first order ODE for $\dot{x}(t)$. Call $\dot{x}$ as $v_x$. $\Dot{v}_x = \frac{dv_x}{dt}$.
\begin{align}
    \begin{split}
        -c_s v_x &= m\Dot{v}_x\\
...
{{< video https://youtu.be/RMkMK32vqyM title="Solving differential equations numerically in Python" width="70%" >}}
...
{{< video https://youtu.be/LAl3ZUiUpus title="Solution – Problem 03/004" width="70%" >}}
